...
```{r skim coucil 11, fig.width=12, fig.height=8}
skim11 %>%
  filter(var == "council") %>%
  filter(stat == "count") %>%
  filter(!is.na(level)) %>%
...
```{r import_pop_data, warning=FALSE, message=FALSE, results='markdown'}
...
```{r skim_11_subset, results='markup'}
